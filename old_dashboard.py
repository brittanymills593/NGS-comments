...
if selected_disease and gene_input:
    ...
    try:
        ...
        # Filter genes
        filtered_df = df[
            df['Gene'].str.upper().isin(input_genes)
        ].copy()
        ...
        if not filtered_df.empty:
            ...
            st.warning(
                "No comments found for the entered genes in the selected disease."
            )
...
try:
    ...
    if 'Panel' in panel_df.columns and 'Genes' in panel_df.columns:
        ...
        st.error(
            "Expected columns 'Panel' and/or 'Genes' not found in the sheet."
        )
...
st.markdown("---")
st.markdown(
    "### Caveats Lookup (including CHIP and CNV)"
)
...
try:
    ...
    if selected_disease_cnv:
        ...
        region_options = [
            ...
            cnv_df[
                cnv_df["Disease"] == selected_disease_cnv
            ]["Region"]
            .dropna()
            .unique()
            .tolist()
    ...
    if selected_region:
        ...
        if selected_disease_cnv:
            ...
            result = result[
                result["Disease"] == selected_disease_cnv
            ]
...
if input_genes:
    ...
    if "DDX41" in input_genes:
        ...
        st.markdown(
            "[Reference 1: The genetic landscape of germline DDX41 variants]"
            "(https://ashpublications.org/blood/article/140/7/716/485483/"
            "The-genetic-landscape-of-germline-DDX41-variants?guestAccessKey=)"
        )


        st.markdown(
            "[Reference 2: Germ-line DDX41 mutations define a unique subtype]"
            "(https://ashpublications.org/blood/article/141/5/534/486974/"
            "Germ-line-DDX41-mutations-define-a-unique-subtype?guestAccessKey=)"
        )


        st.markdown(
            "[Reference 3: Prevalence and significance of DDX41 gene variants]"
            "(https://ashpublications.org/blood/article/142/14/1185/497190/"
            "Prevalence-and-significance-of-DDX41-gene-variants?guestAccessKey=)"
        )
    ...
    if "RUNX1" in input_genes:
        ...
        st.markdown(
            "[Reference: RUNX1-mutated families phenotype]"
            "(https://ashpublications.org/bloodadvances/article/4/6/1131/"
            "452758/RUNX1-mutated-families-show-phenotype)"
        )
    ...
    if "CALR" in input_genes:
        ...
        st.markdown(
            "[Reference: CALR study]"
            "(https://doi.org/10.1002/ajh.25065)"
        )
